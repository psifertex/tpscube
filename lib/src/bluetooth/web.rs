...
impl BluetoothCube {
    ...
    async fn request_and_connect(
        ...
        *connected_name.lock().unwrap() = Some(device_name.clone());
        ...
        // For GAN cubes, capture the device key from BLE advertisement
        // manufacturer data via the experimental `watchAdvertisements()`
        // API. This provides the 6-byte salt for the AES cipher.
        //
        // The capture is bounded by a ~2s timeout internally. On desktop
        ...
        // gan-web-bluetooth library does the same (line 224).
        {
            ...
            let closure = Closure::wrap(Box::new(move |_event: web_sys::Event| {
                ...
                *state_for_disconnect.lock().unwrap() = BluetoothCubeState::Discovering;
                // Drop the device wrapper so polling code sees None.
        ...
        let cube: Box<dyn BluetoothCubeDevice> = match cube_type {
            BluetoothCubeType::GAN => {
                super::gan::gan_web_connect(server, device_name, device_key, listeners.clone()).await?
            }
    ...
    async fn try_capture_manufacturer_data(device: &web_sys::BluetoothDevice) -> Option<[u8; 6]> {
        ...
        let closure = Closure::wrap(Box::new(move |event: web_sys::Event| {
            ...
            if let Some(dv) = mf_raw.dyn_ref::<js_sys::DataView>() {
                ...
                if len >= 11 {
                    // Skip 2-byte CIC + 3-byte header = offset 5, then read 6 bytes
                    let mut key = [0u8; 6];
                    for j in 0..6 {
            ...
            // Standard path: manufacturerData is a BluetoothManufacturerDataMap (Map).
